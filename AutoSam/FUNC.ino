...
float corrTemp (float temp) { // функция принимает текущую температуру
...
float conc_f(float t) { // Определение содержания спирта в кипящей жидкости,%об методом аппроксимации взята с форума http://labspirt.com/forum/index.php/topic,2403.15.html
  float Ti, f;
  Ti = (t - 89) / 6.49;
...
float conc_s(float t) { //Определение содержания спирта в парах,%об методом аппроксимации. Взято с онлайн-калькулятора https://planetcalc.ru/5992/
  float Ti, s;
...
void EEPROM_int_write(int addr, int num) {  // запись int
...
void EEPROM_float_write(int addr, float num) { // запись float
