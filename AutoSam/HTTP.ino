...
//**************************************************************************************************
void handle_Del1() {                             // функция изменения уставок с web страницы //SteamTemp
  ...
  if (delta1 == 0) SetTemp1 = 0;                // устанавливаем уставку 0
  else SetTemp1 = SteamTemp + delta1;            // устанавливаем температуру отключения клапана равной текущей температуре плюс дельта
...
//**************************************************************************************************
void handle_Del2() {                             // функция изменения уставок с web страницы //PipeTemp(2/3)
  ...
  if (delta2 == 0) SetTemp2 = 0;                // устанавливаем уставку 0
  else SetTemp2 = PipeTemp + delta2;            // устанавливаем температуру отключения клапана равной текущей температуре плюс дельта
...
//***************************************************************************************************
void handle_button() {                                 // функция управления клапаном с web страницы
  int state = HTTP.arg("state").toInt();            // получаем от клиента строку с номером нажатой кнопки

  if (state == 6 ){ // если передан номер кнопки 6 "Открыть"
      valveset = true;
  }
  if (state == 7 ){ // если передан номер кнопки 7 "Закрыть"
      valveset = false;
  }
  if (state == 10) { // если передан номер кнопки 10 "Поиск датчиков"
    findds();
  }
...
void handle_setMD() {                             // функция изменения настроек с web страницы (режим)
  modesam = HTTP.arg("modesam").toInt();          // получаем от клиента строку с режимом
  HTTP.send(200, "text/plain", "OK");            // передаём ответ
  EEPROM.write(0, modesam);
  EEPROM.commit();
}
void handle_setMB() {                             // функция изменения настроек с web страницы (Мин. температура)
...
void handle_setMT() {                             // функция изменения настроек с web страницы (Макс. температура бака)
  maxTank = HTTP.arg("maxtank").toFloat();       // получаем от клиента строку с максимальной температурой бака
  HTTP.send(200, "text/plain", "OK");            // передаём ответ
  EEPROM_float_write(9, maxTank);
}

void handle_setSM() {                             // функция изменения настроек с web страницы
...
void handle_setD0() {                             // функция изменения настроек с web страницы (скорость изменения температуры)
...
void handle_reset() {                             // функция перезагрузки с web страницы
  ESP.restart();
  HTTP.send(200, "text/plain", "OK");            // передаём ответ
}
...
{ String json = "{";                                // начинаем с открывающейся фигурной скобки
  ...
  json += "\",\"VS\":\"" + String();
  if (digitalRead(valve)) json += " ЗАКРЫТ "; else json += " ОТКРЫТ ";
